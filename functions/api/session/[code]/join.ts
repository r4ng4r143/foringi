...
export const onRequestPost: PagesFunction<Env> = async ({ env, params, request }) => {
  try {
    ...
    if (!body.players || !Array.isArray(body.players) || body.players.length === 0) {
      return Response.json({ error: 'At least one player required' }, { status: 400 });
    }

    if (body.players.length > 4) {
      return Response.json({ error: 'Maximum 4 players per join' }, { status: 400 });
    }
    ...
    if (playerIds.length === 0) {
      return Response.json({ error: 'No valid players provided' }, { status: 400 });
    }
